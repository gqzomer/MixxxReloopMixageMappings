...
Mixage.init = function (id, debugging) {
	...
	Mixage.beatConnection[2] = engine.makeConnection("[Channel1]", "loop_enabled", function (value) {
		if (value === 1) {
			Mixage.beatConnection[3] = engine.makeConnection("[Channel1]", "beat_active", function (value) {
				if (value === 1) {
					if (engine.getValue("[Channel1]", "beatloop_size") > 0.0625) {
			...
			Mixage.beatConnection[3].disconnect();
			Mixage.toggleLED(OFF, "[Channel1]", "loop");
		}
	});

	var numEffectSlots = engine.getValue("[EffectRack1_EffectUnit1]", "num_effects");
	var numEffectSlots = engine.getValue("[EffectRack1_EffectUnit2]", "num_effects");
...
//********************** Code to copy/paste to your script
...
Mixage.handleBeatMovePressed = function (channel, control, value, status, group) {
	Mixage.isBeatMovePressed[group] = value === 0x7f ? true : false;
};
